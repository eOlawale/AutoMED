        ...
        <div className="p-4 flex-1 overflow-y-auto">
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2 px-2">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">My Garage</h3>
              <button 
            ...
            <div className="space-y-1">
              ...
              {vehicles.length === 0 && (
                <div className="text-sm text-slate-400 px-3 py-4 text-center border-2 border-dashed border-slate-200 rounded-lg">
                  No vehicles yet
                </div>
              )}
            </div>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 sticky top-0 z-20">
          <div className="px-6 py-4 flex items-center justify-between">
            <h1 className="text-xl font-bold text-slate-800">{currentView}</h1>
            {activeVehicle && (
              <div className="flex items-center gap-3">
                <span className={`h-2.5 w-2.5 rounded-full ${brandColor.replace('bg-', 'bg-')}`}></span>
                <div className="text-right hidden sm:block">
                  <p className="text-sm font-semibold text-slate-900 leading-tight">{activeVehicle.year} {activeVehicle.brand} {activeVehicle.model}</p>
                  <p className="text-xs text-slate-500">{activeVehicle.fuelType} • {activeVehicle.mileage.toLocaleString()} mi</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="p-4 md:p-6 max-w-5xl mx-auto">
          {!activeVehicle ? (
            <div className="text-center py-20">
              ...
              <h2 className="text-xl font-bold text-slate-800 mb-2">Welcome to AutoMate</h2>
              <p className="text-slate-500 mb-6 max-w-md mx-auto">
                Get started by adding your first vehicle to track maintenance, diagnose issues, and get expert advice.
              </p>
              <button 
                onClick={() => setShowAddModal(true)}
                className="bg-brand-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-brand-700 transition-all shadow-lg shadow-brand-200"
              >
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="bg-white border border-slate-200 rounded-xl p-4">
                       <h3 className="font-bold text-slate-800 mb-2">Common Issues</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-lg font-bold text-slate-800 mb-4">DIY Maintenance Guides</h2>
        ...
        <div className="grid md:grid-cols-3 gap-4">
           {['Change Engine Oil', 'Replace Air Filter', 'Check Brake Pads', 'Top up AdBlue', 'Reset Service Light'].map(task => (
             <button 
               ...
               className="p-4 bg-white border border-slate-200 rounded-lg hover:border-brand-500 text-left transition-colors"
             >
